...
__global__ void reduce3(float *in, float *out, int n)
{
    ...
    sdata[tid+blocksize] = ((i + n/2)<n) ? in[i+n/2] : 0;
sdata[tid] = (i < n) ? in[i] : 0;
    __syncthreads();
 //int s = blocksize*2;
   for(unsigned int j=blocksize ;j>0;j=j/2){
		if((tid+j)<(j*2)){
	   	sdata[tid] += sdata[tid+j]; //sum number stored in low index	
...
{    
   ...
   dim3 block(tile_width/2, 1, 1); //half here
